...
/// Latest version at: https://pastebin.com/nphdEi1zdi </summary>
...
public class CmdLine : MonoBehaviour {
	...
	public void PopulateWithBasicCommands () {
		...
		addCommand ("reload", (args) => {
			SceneManager.LoadScene (SceneManager.GetActiveScene ().buildIndex);
		}, "reloads current scene.");
		addCommand ("pref", (args) => {
			for (int i = 1; i < args.Length; ++i) {
				log (args [i] + ":" + PlayerPrefs.GetString (args [i]));
			}
...
//		}, "clears player prefs.");
	...
	public InitialColorSettings colorSettings = new InitialColorSettings();
	...
	public void SetOverlayModeInsteadOfWorld(bool useOverlay) {
		if (useOverlay && mainview.renderMode != RenderMode.ScreenSpaceOverlay) {
			...
			worldSpaceSettings.DoIt (mainview);
		}
	}
	private Canvas CreateUI () {
		...
		img.color = colorSettings.background;
		...
		tmpText.color = colorSettings.text;
		tmpInputField.selectionColor = colorSettings.userSelection;
		tmpInputField.customCaretColor = true;
		tmpInputField.caretColor = colorSettings.userInput;
		...
		if (tmpInputField.verticalScrollbar.handleRect == null) {
			...
			bimg.color = colorSettings.scrollbar;
	...
	public void SetInteractive(bool enableInteractive) {
		...
		if (enableInteractive) {
			...
			if (disabledUserControls == currentlyActiveCmdLine) {
				...
				if(disabledUserControls.onStopInteraction != null) disabledUserControls.onStopInteraction.Invoke ();
	...
	private class CmdLineValidator : TMP_InputValidator {
		...
		public bool HasProperInputTags(string text) {
			...
			if (tags.Count == 0 || !tags.Contains ("noparse"))
				return false;
			string colorTag = "#" + CmdLine.ColorToHexCode (cmd.colorSettings.userInput);
			return tags.Contains(colorTag);
		}
		public bool UpdateIfUserInputTagsArePresent(string text) {
		...
		public string BEGIN_USER_INPUT() {
			return "<#" + CmdLine.ColorToHexCode (cmd.colorSettings.userInput) + "><noparse>";
		}
		public string END_USER_INPUT() { return "</noparse></color>"; }
		int EndUserInputIfNeeded(ref string text) {
			int added = 0;
			if (isUserEnteringInput) {
		...
		public override char Validate(ref string text, ref int i, char c) {
			...
			// if the user wants to execute (because they pressed enter)
				...
				if (cmd.waitingToReadLine != null) {
					cmd.waitingToReadLine (cmd.GetUserInput ());
					cmd.waitingToReadLine = null;
				} else if (cmd.onInput != null) {
					cmd.onInput (cmd.GetUserInput ());
				} else {
	...
	public void setText (string text) {
		...
		if (inputvalidator != null) {
			inputvalidator.UpdateIfUserInputTagsArePresent (text);
		}
	}
	...
	void OnDrawGizmos() {
		...
		Gizmos.color = colorSettings.background;
		for (int i = 0; i < points.Length; ++i) {
			Gizmos.DrawLine (points [i], points [(i + 1) % points.Length]);
		}
	}
	#endregion // Unity Editor interaction
	#region find-and-disable FPS
	[Tooltip("Will search for a MonoBehaviour with the given complete name to disable when the commandline is active. Example: \"UnityStandardAssets.Characters.FirstPerson.FirstPersonController\". If empty, will check FPS controllers from Standard Assets")]
	public string characterControllerClassName = "";
	private Type controllerType;
	private GameObject playerControllerObject;
	public event DoAfterVisiblityChange onStopInteraction, onStartInteraction;
	public UnityEngine.Events.UnityEvent onActiveEvent;
	public UnityEngine.Events.UnityEvent onInactiveEvent;

	private static MonoBehaviour FindMonobehaviorOfClass(string className, out Type type) {
		if(className == "") { type = null; return null; }
		type = GetType(className);
		if (type == null) return null;
		return GameObject.FindObjectOfType (type) as MonoBehaviour;
	}
	private void SetupControllerDisable () {
		MonoBehaviour mb = FindMonobehaviorOfClass (characterControllerClassName, out controllerType);
		if (mb == null) {
			mb = FindMonobehaviorOfClass ("UnityStandardAssets.Characters.FirstPerson.FirstPersonController", out controllerType);
			if (mb == null) {
				mb = FindMonobehaviorOfClass ("UnityStandardAssets.Characters.FirstPerson.RigidbodyFirstPersonController", out controllerType);
			}
		}
		if (mb != null) {
			playerControllerObject = mb.gameObject;
			onStartInteraction += DisableIfCmd;
			onStopInteraction += DisableIfCmd;
		}
	}
	private void DisableIfCmd() {
		SetPlayerControl (!IsInteractive ());
	}
	private void SetPlayerControl(bool enabled) {
		MonoBehaviour fps = playerControllerObject.GetComponent (controllerType) as MonoBehaviour;
		if (fps) {
			fps.enabled = enabled;
	...
	#endregion // find-and-disable FPS
	...
	void Update () {
		...
		if (currentlyActiveCmdLine == this && disabledUserControls != this) {
			...
			if (onStartInteraction != null) onStartInteraction.Invoke ();
		}
	}

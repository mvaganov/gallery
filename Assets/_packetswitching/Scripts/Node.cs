...
public class Node : MonoBehaviour {
	public class NeighborGate : MonoBehaviour {
		...
		public static LineRenderer MakeRing(ref GameObject gameObject, Node self) {
			LineRenderer lr = Lines.MakeArc(ref gameObject, 340, 24, Vector3.up, startAngle*self.GetLineRadius(), 
		...
		public void CalculateCircle(Node self, int index, int connectionCount) {
			GameObject go = gameObject;
			MakeRing(ref go, self);
			transform.SetParent(self.transform);
			transform.localPosition = Vector3.zero;
		}
		public static NeighborGate Create(Node self, Node n, float linesize, Edge edge){
			GameObject go = null;
	...
	public NeighborGate GetNeighbor(Node n) {
		...
		if (timer >= settings.switchLag) {
			if (messageToSend != null && messageToSend.destination != null && messageToSend.destination != this) {
				...
				&& (packetToSend = FirstPacketWithin (settings.inventoryRadius)) != -1) {
					...
					Node next = edges [neighborToGoTo].Other(this);
					p.SwitchTo (next);
					timer = 0;
				}
			}
	...
	Edge AddNeighbor(Node otherNode, bool forced, Edge edge) {
		if(forced 
	...
	void OnTriggerStay(Collider c) {
		...
		if(other != null && c.gameObject != gameObject) {
			...
			Edge e = this.AddNeighbor (other, forcedNeighbor, null);
			if (e != null) {
